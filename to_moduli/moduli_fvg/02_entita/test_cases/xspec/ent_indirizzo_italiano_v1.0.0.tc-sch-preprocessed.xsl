<?xml version="1.0" encoding="UTF-8"?><xsl:stylesheet xmlns:eii="http://entita/indirizzo_italiano" xmlns:iso="http://purl.oclc.org/dsdl/schematron" xmlns:saxon="http://saxon.sf.net/" xmlns:schold="http://www.ascc.net/xml/schematron" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xml:base="file:/F:/softwareDEV/GIT/repos/suap-gt-specifiche/to_moduli/moduli_fvg/02_entita/ent_indirizzo_italiano_v1.0.0.sch" version="2.0"><!--Implementers: please note that overriding process-prolog or process-root is 
...
<xsl:template match="/"><svrl:schematron-output xmlns:svrl="http://purl.oclc.org/dsdl/svrl" title="" schemaVersion=""><xsl:comment><xsl:value-of select="$archiveDirParameter"/>   
		 ...
		 <xsl:value-of select="$fileDirParameter"/></xsl:comment><svrl:ns-prefix-in-attribute-values uri="http://entita/indirizzo_italiano" prefix="eii"/><svrl:active-pattern><xsl:attribute name="document"><xsl:value-of select="document-uri(/)"/></xsl:attribute><xsl:attribute name="id">indirizzo_italiano</xsl:attribute><xsl:attribute name="name">indirizzo_italiano</xsl:attribute><xsl:apply-templates/></svrl:active-pattern><xsl:apply-templates select="/" mode="M1"/></svrl:schematron-output></xsl:template>

<!--SCHEMATRON PATTERNS-->


<!--PATTERN indirizzo_italiano-->
<xsl:variable name="keysComuni" select="document('../01_vocabolari/voc_ComuniItaliani_cl.xml')//Row"/>

	<!--RULE -->
<xsl:template match="eii:indirizzo_italiano" priority="1000" mode="M1"><svrl:fired-rule xmlns:svrl="http://purl.oclc.org/dsdl/svrl" context="eii:indirizzo_italiano"/><xsl:variable name="comune" select="normalize-space($indirizzo_italiano/comune)"/>

		<!--ASSERT -->
<xsl:choose><xsl:when test="count($keysComuni[                 normalize-space(Value[@ColumnRef='code' ]/SimpleValue) = $comune                 ]) = 1"/><xsl:otherwise><svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl" test="count($keysComuni[ normalize-space(Value[@ColumnRef='code' ]/SimpleValue) = $comune ]) = 1"><xsl:attribute name="id">ass_comune_cl_check</xsl:attribute><xsl:attribute name="location"><xsl:apply-templates select="." mode="schematron-select-full-path"/></xsl:attribute><svrl:text>
                
                Comune non esiste
                <xsl:text/><xsl:value-of select="$comune"/><xsl:text/> 
            </svrl:text></svrl:failed-assert></xsl:otherwise></xsl:choose><xsl:apply-templates select="*|comment()|processing-instruction()" mode="M1"/></xsl:template><xsl:template match="text()" priority="-1" mode="M1"/><xsl:template match="@*|node()" priority="-2" mode="M1"><xsl:apply-templates select="*|comment()|processing-instruction()" mode="M1"/></xsl:template></xsl:stylesheet>